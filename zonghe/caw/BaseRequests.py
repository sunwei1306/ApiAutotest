...
class BaseRequests:
    ...
    def get(self,url,**kwargs):
        '''
        封装reques中的get方法，增加打印，正价异常处理，使用session发送请求
        :param url:
        :param kwargs:
        :return:
        '''
    ...
    def post(self,url,data=None,json= None,**kwargs):
        try:
            ...
            print(f"发送post请求，url：{url},请求参数data：{data},json:{json},其他：{kwargs}")
            return r
...
if __name__ == '__main__':
    r = BaseRequests().get("http://www.httbin.org?get?name=root")
    print(r.text)
    cs = {"user":"root","pwd":123456}
    r = BaseRequests().post("http://www.httbin.org/post",data= cs)
    print(r.text)